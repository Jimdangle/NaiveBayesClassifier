...
# Student Name: Cal Blanco
# UW Email    : wcblanco@ucsc.edu
...
class NaiveBayes():
    ...
    def word_set(self, filename:list):
        ...
        with open(filename, 'r') as f:
            text = f.read()
            text = text[text.find("\n")+1:] #find the first newline char and split (to remove subject line)

            text = text.replace('\r', '') #replace returns 
            text = text.replace('\n', ' ') #replace newlines 
            text = text.lower() #lowercase everything
            

            words = text.split(' ') #split into individual strings
            

            return set(words) #return the unique keys 
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        self.num_train_hams = len(train_hams) #get number of ham emails 
        self.num_train_spams = len(train_spams) #get number of spam emails 
        ...
        def get_counts(filenames:list):
            ...
            self.word_counts_ham!
            """
            W = list() #list of all words   
            for file in filenames: #build Total Word count for list of files 
                w = self.word_set(file) #load the words from a email into a list w 
                W.extend(w) #extend our main list by w 
            
            out_count = dict() #word count given filenames 

            for word in W: #build dict 
                if word in out_count.keys():
                    out_count[word] +=1 #increment found
                else:
                    out_count[word] = 1 #initialize new word 


            return out_count

        self.word_counts_ham = get_counts(train_hams) #get ham word count
        self.word_counts_spam = get_counts(train_spams) #spam count 
    ...
    def predict(self, filename:str):
        ...
        IW = self.word_set(filename)  #get the word set of the input file 

        SpamQuantity = np.log(self.num_train_spams) 
        HamQuantity = np.log(self.num_train_hams) 

        for word in IW:
            temp_spam = self.word_counts_spam.get(word,0) +1 #get numerator for P(word|spam) and P(word|ham)
            temp_ham = self.word_counts_ham.get(word,0) +1 

            temp_spam = temp_spam / (self.num_train_spams+2) # divide numerator by num of words in spam and ham
            temp_ham = temp_ham / (self.num_train_hams+2)

            SpamQuantity += np.log(temp_spam) #add log of probability to quantity (because of log properties this is addition)
            HamQuantity += np.log(temp_ham)
        
        if SpamQuantity > HamQuantity: #numerator for spam is greater 
